...
class Admin extends CI_Controller
{
    ...
    public function generate_transcript_pdf1($id)
    {
        $id = base64_decode($id);
    ...
    public function generate_transcript_pdf_preview($id)
    {
        if ($this->session->userdata('logged_in')) {
            ...
            foreach ($sem_order as $sem) {
                ...
                //     }
                foreach ($grouped_courses as $course_code => $course_records) {
                
                    usort($course_records, function($a, $b) {
                        return $a->id - $b->id;  
                    });

                    
                    $grouped_courses[$course_code] = $course_records;
                }
     ...
     public function generate_transcript_pdf($encid)
    {
